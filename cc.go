...
var cc = template.Must(template.New("cc").Funcs(template.FuncMap{
	...
	"shouldSleep": func(row []byte, i int) bool {
		if len(row) >= sleepThreshold {
			return true
		}
		...
		fmt.Println(((i-1)*len(row))%sleepThreshold, sleepThreshold-len(row))
		return ((i-1)*len(row))%sleepThreshold >= sleepThreshold-len(row)
	},
