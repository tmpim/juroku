...
RUN CGO_CFLAGS_ALLOW=".*" CGO_LDFLAGS_ALLOW=".*" go build -ldflags '-extldflags "-fno-PIC -static -Wl,--no-as-needed -lm -ldl"' -buildmode pie -tags 'osusergo netgo static_build' -o juroku ./stream/server
